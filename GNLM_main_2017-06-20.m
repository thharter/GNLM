...
% Update June 20, 2017:
%    corral NgwDirect rate adjusted by factor 0.669356 to account for rasterization of actual corral area
%    lagoon NgwDirect rate adjusted by factor 0.373176 to account for rasterization of actual lagoon area
%    logical error correction: replace 3599 with 3999 and 6599 with 6999
%    logical error correction: move assignment of NgwDirect for 1600 and 1601 outside of Period loop
%    updated input filename for LanduseTable to "LanduseTable_2017_0515.xlsx"
...
LandUseFile = 'LanduseTable_2017_0515.xlsx';
...
%% Set NgwDirect for alfalfa and clover in simulated period
...
if iyear > 2; NgwDirect(DirApp(:,year_col) == 2) = 1171 * 0.373176; end % old value 182.5; adjustment to account for rasterization which increased mapped area
% Corrals
if iyear > 2; NgwDirect(DirApp(:,year_col) == 3) = 183 * 0.669356; end % adjustment to account for rasterization, which increased mapped area
% Golf courses
...
for i = 1:length(DirAppFac.FACID)
    ...
    if DirAppFac.FACID(i,1) >= 3501 && DirAppFac.FACID(i,1) <= 3999
        NlandApplied(pixels_per_fac_id, 1) = TotalPeopleRatio(iyear) * DirAppFac.EFF_KgN_Y(i,1) / (0.25*length(pixels_per_fac_id));
    end
    ...
    % FP PercBasins
    if DirAppFac.FACID(i,1) >= 6500 && DirAppFac.FACID(i,1) <= 6999
        NgwDirect(pixels_per_fac_id, 1) = TotalPeopleRatio(iyear) * DirAppFac.PERC_KgN_Y(i,1) / (0.25*length(pixels_per_fac_id));
    end
end
